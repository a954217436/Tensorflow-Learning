...
W1 = tf.Variable(1 * tf.truncated_normal([784, 250], mean=0.0, stddev=0.1))
b1 = tf.Variable(tf.zeros([250])+0.1)
L1 = tf.nn.relu(tf.matmul(x, W1) + b1)
#prediction = tf.nn.softmax(tf.matmul(x, W1) + b1)

##加一层后，L1的输出应该是relu等激励函数，不能使用softmax
W2 = tf.Variable(1 * tf.truncated_normal([250, 10], mean=0.0, stddev=0.1))
